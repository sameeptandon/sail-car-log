from os.path import join as pjoin
from pipeline_utils import dset_dir_from_rss, print_and_call
from pipeline_config import CAMERA, NUM_CPUS
from graphslam_config import MATCH_JSON_DATA, GRAPHSLAM_OPT_POS_DIR, GRAPHSLAM_LANES_DIR, GRAPHSLAM_VIDEOS_DIR, GRAPHSLAM_ALIGN_DIR
from joblib import Parallel, delayed


if __name__ == '__main__':
    cmds = list()
    for match in MATCH_JSON_DATA:
        video_file = pjoin(dset_dir_from_rss(match['rss1']), '_'.join(match['rss1'][1:]) + '%d.avi' % CAMERA)
        fstem1 = '--'.join(match['rss1'])
        fstem2 = '--'.join(match['rss2'])
        fname1 = fstem1 + '.npz'
        fname2 = fstem2 + '.npz'
        outvideo = pjoin(GRAPHSLAM_VIDEOS_DIR, '+'.join((fstem1, fstem2)) + '.avi')
        tb = pjoin(GRAPHSLAM_ALIGN_DIR, '+'.join((fstem1, fstem2)) + '.h5')
        cmd = 'python project_map_on_video.py {vid} {ldr1} {ldr2} {gps1} {gps2} {match} {t1} {t2} {tb} {out}'.format(
                vid=video_file, ldr1=pjoin(GRAPHSLAM_LANES_DIR, fname1), ldr2=pjoin(GRAPHSLAM_LANES_DIR, fname2),
                gps1=match['gps_file1'], gps2=match['gps_file2'], match=match['match_file'],
                t1=pjoin(GRAPHSLAM_OPT_POS_DIR, fname1), t2=pjoin(GRAPHSLAM_OPT_POS_DIR, fname2), tb=tb, out=outvideo)
        cmds.append(cmd)
    #print cmds
    Parallel(n_jobs=NUM_CPUS-1)(delayed(print_and_call)(cmd) for cmd in cmds)
