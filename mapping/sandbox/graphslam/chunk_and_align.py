...
if __name__ == '__main__':
    for d in MATCH_JSON_DATA:
        ...
        cmd = 'pcl_concatenate_points_pcd %s' % ' '.join(chunk1_files)
        print_and_call(cmd)
        cmd = 'mv output.pcd %s' % merged_chunks1
        print_and_call(cmd)

        cmd = 'pcl_concatenate_points_pcd %s' % ' '.join(chunk2_files)
        print_and_call(cmd)
        cmd = 'mv output.pcd %s' % merged_chunks2
        print_and_call(cmd)
