...
void caffeCallback(const sensor_msgs::ImageConstPtr& msg){
  ...
  //cv::Mat frame = cv::imread("/home/jpazhaya/caffe/examples/images/cat.jpg");
...
//   std::copy(preprocessed_frame.data,preprocessed_frame.data+frameSize*3,(float*)input_data);
...
//   cv::Mat pixel_mask(480,640,CV_8UC3,cv::Scalar::all(0));
// //   const float* pixel_pred = result[1]->cpu_data();
...
//           cv::rectangle(pixel_mask,cv::Point((i*4+k)*8,(j*4+m)*8),cv::Point((i*4+k+1)*8,(j*4+m+1)*8),cv::Scalar(0,0,pixel_pred->data_at(m,k,j,i)*255),CV_FILLED);
        }
      }
    }
  }
  ...
  float bb_pts[4];
//   for(int i= 0;i<16;i++){
//     std::cout<< bb_pred->cpu_data()[i*(20*15)]<< " , " << bb_pred->cpu_data()[i*(20*15)+(20*15*16)]<< " , " << bb_pred->cpu_data()[i*(20*15)+(20*15*16)*2]<< " , " << bb_pred->cpu_data()[i*(20*15)+(20*15*16)*3] << "\n";
//   }
//   std::cout<<"\n";
//   int sz[] = {60,80,4};
//   cv::Mat bb(3,sz,CV_32F,cv::Scalar::all(0));
  for(int i=0;i<20;i++){
    ...
    for(int j=0;j<15;j++){
      ...
      for(int k=0;k<4;k++){
        for(int m=0;m<4;m++){
//           bb.at<float>(j*4+m,i*4+k,0) = bb_pred->data_at(0,k+m*4,j,i)+x_offset;
//           bb.at<float>(j*4+m,i*4+k,1) = bb_pred->data_at(1,k+m*4,j,i)+y_offset;
//           bb.at<float>(j*4+m,i*4+k,2) = bb_pred->data_at(2,k+m*4,j,i)+x_offset;
//           bb.at<float>(j*4+m,i*4+k,3) = bb_pred->data_at(3,k+m*4,j,i)+y_offset;
...
//             std::cout<< bb_pts[0]-x_offset << " , " << bb_pts[1]-y_offset << " , " << bb_pts[2]-x_offset << " , " << bb_pts[3]-y_offset << "\n";
//             std::cout<< bb_pts[0] << " , " << bb_pts[1] << " , " << bb_pts[2] << " , " << bb_pts[3] << "\n";
...
//   for(int i=0;i<80;i++){
//     for(int j=0;j<60;j++){
//       if(hard_mask.at<cv::Vec3b>(j,i)[2] > 0){
//          cv::rectangle(frame,cv::Point(bb.at<float>(j,i,0),bb.at<float>(j,i,1)),cv::Point(bb.at<float>(j,i,2),bb.at<float>(j,i,3)),cv::Scalar(255,0,0));
// //          cv::rectangle(frame,cv::Point(i*8,j*8),cv::Point(i*8+8,j*8+8),cv::Scalar(255,0,0));
//       }
//     }
//   }
...
{
  ros::init ( argc,argv,"MultiLanePredictionNode" );
  ros::NodeHandle nh_;
