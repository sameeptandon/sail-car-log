...
void GPSLogger::safeWrite(Serial* port, string cmd) {
...
string GPSLogger::safeRead(Serial* port) {
    string buf = port->readline();
    cout << buf << endl;
    return buf;
}

Serial* GPSLogger::Connect(string port) {
    int baudrate = 115200;
...
void GPSLogger::Run(Serial* port) {
