...
if __name__ == '__main__':
    ...
    right_points = np.zeros((0, 6))
    left_points = np.zeros((0, 6))
    ...
    while True:
        ...
        right_point = np.array([-1, -1, -1, 0, count, 0.0])
        left_point = np.array([-1, -1, -1, 0, count, 0.0])
        if np.max(O_bleft) > 0:
            left_z = ((bottom_left[0]-cam['cv'])*np.sin(pitch)*height + f*cos(pitch) * height)/(cos(pitch)*(bottom_left[0]-cam['cv']) - f*sin(pitch))
            left_x = (cos(pitch)*left_z-sin(pitch)*height)*(bottom_left[1]-cam['cu'])/f
            left_y = 0.0
            pos = np.array([left_x, left_y, left_z, 1])
            intermediate = np.linalg.solve(Tc, pos)
            left_point[0:4] = np.dot(tr[count, :, :], intermediate)
            left_point[5] = 1
        if np.max(O_bright) > 0:
            right_z = ((bottom_right[0]-cam['cv'])*np.sin(pitch)*height + f*cos(pitch) * height)/(cos(pitch)*(bottom_right[0]-cam['cv']) - f*sin(pitch))
            right_x = (cos(pitch)*right_z-sin(pitch)*height)*(bottom_right[1]-cam['cu'])/f
            right_y = 0.0
            pos = np.array([right_x, right_y, right_z, 1])
            intermediate = np.linalg.solve(Tc, pos)
            right_point[0:4] = np.dot(tr[count, :, :], intermediate)
            right_point[5] = 1

        if left_point[5] == 0 and np.max(O[:, 2, 2]) > 0:
            actual_x = 4
            actual_y = left_activations * ratio + ratio

            left_z = ((actual_y-cam['cv'])*np.sin(pitch)*height + f*cos(pitch) * height)/(cos(pitch)*(actual_y-cam['cv']) - f*sin(pitch))
            left_x = (cos(pitch)*left_z-sin(pitch)*height)*(actual_x-cam['cu'])/f
            left_y = 0.0
            pos = np.array([left_x, left_y, left_z, 1])
            intermediate = np.linalg.solve(Tc, pos)
            left_point[0:4] = np.dot(tr[count, :, :], intermediate)
        ...
        if right_point[5] == 0 and np.max(O[:, 317, 2]) > 0:
            actual_x = 1276
            actual_y = right_activations * ratio + ratio

            right_z = ((actual_y-cam['cv'])*np.sin(pitch)*height + f*cos(pitch) * height)/(cos(pitch)*(actual_y-cam['cv']) - f*sin(pitch))
            right_x = (cos(pitch)*right_z-sin(pitch)*height)*(actual_x-cam['cu'])/f
            right_y = 0.0
            pos = np.array([right_x, right_y, right_z, 1])
            intermediate = np.linalg.solve(Tc, pos)
            right_point[0:4] = np.dot(tr[count, :, :], pos)
            right_point[5] = 1
