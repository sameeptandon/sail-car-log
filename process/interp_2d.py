...
if __name__ == '__main__':
    ...
    #left[all_time % 10 != 0 ] = -1 
    
    left_time_array = np.where(left[:, 0] != -1)[0]
    right_time_array = np.where(right[:, 0] != -1)[0]

    spline_left_x = scipy.interpolate.UnivariateSpline(left_time_array, left[left_time_array, 0], k=polynomial_fit)
    spline_left_y = scipy.interpolate.UnivariateSpline(left_time_array, left[left_time_array, 1], k=polynomial_fit)
    spline_right_x = scipy.interpolate.UnivariateSpline(right_time_array, right[right_time_array, 0],k=polynomial_fit)
    spline_right_y = scipy.interpolate.UnivariateSpline(right_time_array, right[right_time_array, 1],k=polynomial_fit)
