...
def cloudToPixels(cam, pts_wrt_cam):
    ...
    width = 4
    (pix, J) = cv2.projectPoints(pts_wrt_cam.transpose(), np.array(
        [0.0, 0.0, 0.0]), np.array([0.0, 0.0, 0.0]), cam['KK'], cam['distort'])
...
def lidarPtsToPixels(map_data, imu_transforms_t, T_from_i_to_l, cam):
    ...
    pts_wrt_camera_t = pts_wrt_lidar_t.transpose()[
        :, 0:3] + cam['displacement_from_l_to_c_in_lidar_frame']
...
if __name__ == '__main__':
    ...
    while True:
        ...
        (pix, mask) = lidarPtsToPixels(map_data_copy,
