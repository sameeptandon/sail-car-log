...
def GetQ50CameraParams():
    ...
    for i in [1, 0, 2]:
        ...
        if i == 0: # left camera
        ...
        elif i == 2: # extra middle misaligned camera 
           
            cam[i]['R_to_c_from_l_in_camera_frame'] = cam[1]['R_to_c_from_l_in_camera_frame']
            cam[i]['displacement_from_l_to_c_in_lidar_frame'] = cam[1]['displacement_from_l_to_c_in_lidar_frame']

            # extrinsics parameters for transforming points in right camera frame to this camera
            T = np.array([-0.4044573460148553, -0.0035082054017725045, 0.294621777342546])
            #R = np.array([0.9868763382125351, -0.044230141877358735, 0.155302246042119, 0.05295321414301766, 0.9972167001222463, -0.052486284963041585, -0.15254851748916431, 0.06002122580226606, 0.986471693595438])
            #R = R.reshape((3,3))
            R = euler_matrix(0.051, 0.1445, -0.0115)[0:3,0:3]
            ...
            cam[i]['cu'] = 660.77
            cam[i]['cv'] = 476.28
            cam[i]['distort'] = np.array([-0.17869820143584852, 0.29370281837653306, -0.001708835263536307, -0.00025793899504959847, 0.0])
