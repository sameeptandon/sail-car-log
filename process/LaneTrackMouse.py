  ...
  while True:
    ...
    if framenum%2==1:
      ...
      if x is not None and y is not None:
        I_WARP[y-2:y+2, x-2:x+2,:]=[0,0,255]
        Orig_I[unwarped_point[1]/4-2:unwarped_point[1]/4+2, unwarped_point[0]/4-2:unwarped_point[0]/4+2,:]=[0,0,255]
      #I_WARP = cv2.warpPerspective(I_WARP, P, imsize, flags=cv.CV_WARP_INVERSE_MAP) 
      key = ''
      if display:
        imshow('video', np.concatenate((I_WARP,Orig_I), axis=1))
        if framenum==1: 
            key = (waitKey(4000) & 255)
        else:
            key = (waitKey(frameWaitTime) & 255)
        frameWaitTime = 2#max(int(frameWaitTime / 1.5), 2)
        ...
        if y<imsize[1]:
          y-=30
        y = np.clip(y, 0, imsize[1])
        x = np.clip(x, 0, imsize[0])
        ...
        if key == ord('p'):
          paused = not paused
        if key == ord('a') or key == ord('j'):
          x-=2
        if key == ord('d') or key == ord('l'):
          x+=2
    ...
    if (currentTime - lastTime > 10):
        ...
        print framenum
  export_data = -1*np.ones((len(frame_data),2))
  export_data[:,:] = frame_data
  if Left:
    ...
    if len(export_data)<oldleft.shape[0]:
      oldleft = oldleft[0:export_data.shape[0],:] 
    savemat(out_name, dict(left = oldleft, right=export_data))
